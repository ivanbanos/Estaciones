...
const PorAutomotores = () => {
  ...
  const fetchCupos = async () => {
    let response = await GetCuposPorAutomotores()
    if (response == 'fail') {
  ...
  const descargarReporte = async (event) => {
    ...
    let cuposTabla = [['Cliente', 'Nit/CC', 'Placa', 'Cupo asignado', 'Cupo disponible', 'Debe']]
  ...
  return (
    <>
      ...
      <h1>Reporte de cupos por automotores</h1>
          ...
          <CTableBody>
            {cupos.map((cupo) => (
              <CTableRow key={cupo.Placa}>
                ...
                <CTableHeaderCell>{cupo.placa}</CTableHeaderCell>
