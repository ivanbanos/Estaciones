    ...
    // ...existing code...
    {
        ...
        public async Task<IEnumerable<Modelo.OrdenDeDespacho>> GetOrdenesSinFacturaElectronicaCreditoDirecto(FiltroBusqueda filtroOrdenDeDespacho)
        {
            ...
            var result = ordenes.Where(x => string.IsNullOrEmpty(x.idFacturaElectronica)
                && (
                    (!string.IsNullOrWhiteSpace(x.FormaDePago)
                        && x.FormaDePago.Trim().Equals("Crédito Directo", StringComparison.OrdinalIgnoreCase))
                    ||
                    (!string.IsNullOrWhiteSpace(x.FormaDePago2)
                        && x.FormaDePago2.Trim().Equals("Crédito Directo", StringComparison.OrdinalIgnoreCase))
                ));
        ...
        {
            var lineas = ordenes
                .SelectMany(GetLineasFormaPago)
                .Where(x => x.Total > 0 || x.CantidadCombustible > 0);

            return lineas
                .GroupBy(o => o.FormaPago, StringComparer.OrdinalIgnoreCase)
                .Select(g => new ConsolidadoFormaPago
                {
                    FormaPago = g.Key,
                    CantidadFacturas = g.Count(),
                    CantidadCombustible = g.Sum(x => x.CantidadCombustible),
                    Total = g.Sum(x => x.Total)
                })
                .ToList();
        }

        private IEnumerable<LineaFormaPago> GetLineasFormaPago(Modelo.OrdenDeDespacho orden)
        {
            var formaPago1 = NormalizarFormaPago(orden?.FormaDePago);
            var formaPago2 = NormalizarFormaPago(orden?.FormaDePago2);

            var totalOrden = Convert.ToDecimal(orden?.Total ?? 0d);
            var cantidadOrden = Convert.ToDecimal(orden?.Cantidad ?? 0d);
            var tieneFormaPago2 = !string.IsNullOrWhiteSpace(formaPago2);

            var (total1, total2) = ResolverTotalesFormaPago(orden, totalOrden, tieneFormaPago2);

            if (!tieneFormaPago2)
            {
                yield return new LineaFormaPago
                {
                    FormaPago = formaPago1,
                    Total = total1,
                    CantidadCombustible = cantidadOrden
                };
                yield break;
            }

            var cantidad1 = totalOrden > 0 ? (cantidadOrden * total1) / totalOrden : cantidadOrden;
            var cantidad2 = totalOrden > 0 ? (cantidadOrden * total2) / totalOrden : 0m;

            yield return new LineaFormaPago
            {
                FormaPago = formaPago1,
                Total = total1,
                CantidadCombustible = cantidad1
            };
            ...
            yield return new LineaFormaPago
            {
                ...
                Total = total2,
                CantidadCombustible = cantidad2
            };
        }

        private static (decimal total1, decimal total2) ResolverTotalesFormaPago(Modelo.OrdenDeDespacho orden, decimal totalOrden, bool tieneFormaPago2)
        {
            var total1 = orden?.Total1 ?? 0m;
            var total2 = orden?.Total2 ?? 0m;

            total1 = total1 < 0 ? 0 : total1;
            total2 = total2 < 0 ? 0 : total2;

            if (!tieneFormaPago2)
            {
                return (total1 > 0 ? total1 : totalOrden, 0m);
            }
            ...
            {
                total1 = totalOrden - total2;
            }
            else if (total2 <= 0)
            {
                total2 = totalOrden - total1;
            }
        ...
        {
            return string.IsNullOrWhiteSpace(formaPago) ? "Sin forma de pago" : formaPago.Trim();
        }

        private class LineaFormaPago
        {
