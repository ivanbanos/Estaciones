...
const OrdenesDespacho = () => {
  ...
  const getPaymentAmount = (orden, index) => {
    const rawValue = index === 1 ? orden.total1 ?? orden.Total1 : orden.total2 ?? orden.Total2
    const value = Number(rawValue)
    return Number.isFinite(value) && value > 0 ? value : null
  }

  const getFormaPago = (orden) => {
    const forma1 = (orden.formaDePago || orden.formaPago || orden.FormaDePago || '').trim()
    const forma2 = (orden.formaDePago2 || orden.FormaDePago2 || '').trim()

    if (!forma1 && !forma2) {
      return 'N/A'
    }

    const total1 = getPaymentAmount(orden, 1)
    const total2 = getPaymentAmount(orden, 2)

    const forma1Display =
      (forma1 || 'Sin forma de pago') + (total1 ? ` (${formatCurrency(total1)})` : '')

    if (!forma2) {
      return forma1Display
    }
    ...
    const forma2Display =
      ...
      (total2
        ...
        : total1
        ? ` (${formatCurrency(Math.max((orden.total || 0) - total1, 0))})`
        : '')

    return `${forma1Display} + ${forma2Display}`
  }
  ...
  const filteredOrdenes = ordenes
    .filter(
      (orden) =>
        ...
        getFormaPago(orden).toLowerCase().includes(searchTerm.toLowerCase()),
    )
    .filter((orden) => {
                    ...
                    <CTableBody>
                      {currentItems.map((orden) => (
                        <CTableRow key={orden.guid || orden.idVentaLocal}>
                          ...
                          <CTableDataCell>
                            <small>{getFormaPago(orden)}</small>
                          </CTableDataCell>
                          <CTableDataCell>
