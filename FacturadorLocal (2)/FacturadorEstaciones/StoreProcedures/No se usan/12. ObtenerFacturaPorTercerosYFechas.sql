...
CREATE OR ALTER procedure [dbo].[ObtenerFacturaPorTercerosYFechas]
(
	@terceroId int,
...
begin try
	...
	inner join Estacion.dbo.VENTAS on FacturasPOS.ventaId = VENTAS.CONSECUTIVO
	where (@terceroId is null or @terceroId = terceroId)
	AND (@FechaInicio is null or @FechaInicio <= FacturasPOS.fecha)
	AND (@FechaFinal is null or @FechaFinal >= FacturasPOS.fecha)
